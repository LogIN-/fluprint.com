...
export default {
    ...
    methods: {
        ...
        updateNodes() {
            ...
            const allNodesPromise = toPromise(
                allNodes
                    .transition()
                    .duration(this.duration)
                    .attr("transform", d => translate(d, layout))
                    .attr("opacity", 1)
            );
            ...
            text.each(function(d) {
                tentative.push({
                    node: this,
                    data: d,
                    pos: transformNode(d.x, this.getComputedTextLength() + 6)
                });
            });

            const getMaxNode = position => {
                const mapped1 = tentative.map(el => ({
                    node: el.node,
                    x: el.data.x,
                    value: Math.abs(el.pos[position])
                }));
            ...
            if (
                this.textContraint &&
                this.textContraint.xExtreme.value === xExtreme.value &&
        ...
        updateLinks() {
            ...
            const line = layout.getLine(d3).curve(d3.curveBundle.beta(0.75));
            ...
            const promise = toPromise(
                allEdges
                    .transition()
                    .duration(this.duration)
                    .attr("d", d => roundPath(line(d.source.path(d.target))))
            );
        ...
        packageImports(nodes) {
            ...
            nodes.forEach(d => {
                ...
                if (d.data.key) {
                    if (this.counts.hasOwnProperty(d.data.key) === true) {
                        this.counts[d.data.key] += 1;
                    } else {
                        this.counts[d.data.key] = 1;
                    }
                }
        ...
        onData(data) {
            ...
            const root = d3.hierarchy(this.packageHierarchy(data));
            this.links = this.packageImports(root.descendants());
    ...
    watch: {
        ...
        highlightedNode(newCurrent, oldCurrent) {
            ...
            this.$emit("highlightedNodeChanged", {
                new: newCurrent,
                old: oldCurrent
            });
        }
